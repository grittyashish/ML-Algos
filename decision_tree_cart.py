...
#Heuristic : Gini Index : The lower the Gini index, the best
#Gini Index produces Binary Splits only
#Criteria for leaf node : If any kind of split does not result in gini index lower than that of the node itself => Leaf Node
...
def gini_index(X, y, feature):
    if isinstance(X, pd.DataFrame) and isinstance(y, pd.Series):
        if X[feature].dtype.name is 'category':
            gini_index = 0
            for category in X[feature].cat.categories:
                gini = 0
                observation = y.iloc[X[X[feature] == category].index.values]
                if len(observation) == 0:
                    continue
                count = Counter(observation.value_counts())
                for key, value in count.items():
                    gini += (value/len(observation))**2
                #Calculating Gini Index : Weighted Average of gini on all categories
                gini_index += gini*(len(X[X[feature] == category])/len(observation))
            return gini_index
        #If continuous feature
        else:
            sorted_feature = sorted(X[feature].unique()[:-1])
            average_weight_adjacent_rows = [ (sorted_feature[i] + sorted_feature[i+1])/2 for i in range(len(sorted_feature) - 1)]
            best_gini_index = (None,1)
            #Calculating gini index for each average_weight
            for wt in average_weight_adjacent_rows:
                observation_less_than = y.iloc[X[X[feature] < wt].index.values]
                observation_greater_than_equal_to = y.iloc[X[X[feature] >= wt].index.values]
                less_than_gini = 0
                greater_than_equal_to_gini = 0
                less_than_count = Counter(observation_less_than.value_counts())
                greater_than_equal_to_count = Counter(observation_greater_than_equal_to.value_counts())
                for key, value in less_than_count.items():
                    less_than_gini += (value/len(observation_less_than))**2
                for key, value in greater_than_equal_to_count.items():
                    greater_than_equal_to_gini += (value/len(observation_greater_than_equal_to))**2
                gini_index = 1 - less_than_gini - greater_than_equal_to_gini
                if gini_index < best_gini_index[1]:
                    best_gini_index = (wt, gini_index)
            return best_gini_index
    else:
        raise ValueError("Wrong Input Given For Gini Index")
...
class Node:
    ...
    def __init__(self, X, y):
        ...
        #When gini split is 0 or pure split
    ...
    def get_best_feature(self, X, y):
        ...
        for feature in X.columns:
            gini_index_ = gini_index(X, y, feature)
            if not isinstance(gini_index_, tuple):
                if gini_index_ < min_:
                    min_ = gini_index_
                    best_feature = feature

            #If continuous feature=> gini_index_ is of type (best_threshold, gini_index)
            else:
                if gini_index_[1] < min_:
                    min_ = gini_index_[1]
                    best_feature = (feature, gini_index_[0])
        if min_ == 0:
            return None
        return best_feature
